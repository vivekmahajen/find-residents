...
let db = { users: {}, sessions: {}, resets: {}, leads: {}, facilities: {}, records: {} };
let loaded = false;
...
async function createLead(lead) {
  const id = crypto.randomUUID();
  const now = Date.now();
  const rec = { id, ...lead, createdAt: now, updatedAt: now };
  db.leads[id] = rec;
  save();
  return rec;
}
...
async function createFacility(fac) {
  ...
  db.facilities[id] = rec;
  save();
  return rec;
}
// Returns the agency's own facilities plus any shared seed (agencyId null).
async function listFacilities(agencyId) {
  return Object.values(db.facilities)
    .filter((f) => f.agencyId === agencyId || f.agencyId == null)
    .sort((a, b) => String(a.name).localeCompare(String(b.name)));
}
async function getFacility(id) {
  return db.facilities[id] || null;
}
async function updateFacility(id, patch) {
  const f = db.facilities[id];
  if (!f) return null;
...
async function deleteFacility(id) {
  if (!db.facilities[id]) return false;
  delete db.facilities[id];
  save();
  return true;
}

// Generic per-agency records (CRM: contacts, tasks, activities, sequences,
// enrollments, unsubscribes, events).
async function createRecord({ agencyId, kind, data }) {
  const id = crypto.randomUUID();
  const now = Date.now();
  const rec = { id, agencyId, kind, data: data || {}, createdAt: now, updatedAt: now };
  db.records[id] = rec;
  save();
  return rec;
}
async function listRecords(agencyId, kind) {
  return Object.values(db.records)
    .filter((r) => r.agencyId === agencyId && r.kind === kind)
    .sort((a, b) => b.createdAt - a.createdAt);
}
async function listAllRecords(kind) {
  return Object.values(db.records).filter((r) => r.kind === kind);
}

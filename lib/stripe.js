...
// Add accrued usage overage as an invoice item on the customer; it bills on
// their next Stripe invoice.
async function addOverageInvoiceItem({ customerId, amountUsd, description }) {
  const stripe = getStripe();
  return stripe.invoiceItems.create({
    customer: customerId,
    amount: Math.round(Number(amountUsd) * 100),
    currency: 'usd',
    description: description || 'Usage overage',
  });
}

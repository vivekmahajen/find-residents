...
// Produce a record that is SAFE TO PERSIST: sensitive identifiers scrubbed from
// free text, full DOB dropped (age kept). Contact fields are retained (the
// agency legitimately holds them for clients it's working) and role-masked only
// at view time. Returns the storable record + the categories that were removed.
function toStorableRecord(record) {
  const r = record && typeof record === 'object' ? record : {};
  const withheld = new Set();
  const out = {};

  // Free-text fields: scrub Rule-1 identifiers.
  const textFields = ['reference', 'name', 'mailingArea', 'contactName', 'location', 'careFeatures', 'needs', 'notes', 'raw'];
  for (const f of textFields) {
    const v = val(r[f]);
    if (!v) continue;
    const s = scrub(v);
    s.cats.forEach((c) => withheld.add(c));
    out[f] = s.text;
  }

  // Controlled / contact fields: store as-is (never scrub a phone as a number).
  const plainFields = ['gender', 'phone', 'email', 'contactRelationship', 'contactPhone', 'carePreferenceType', 'roomType', 'language', 'budgetAmount', 'payor', 'shareOfCost', 'timeline'];
  for (const f of plainFields) {
    const v = val(r[f]);
    if (v) out[f] = v;
  }

  // DOB → age; never persist the date of birth.
  const dobProvided = !!val(r.dob);
  const age = ageFromDob(r.dob) || val(r.age);
  if (age) out.age = age;
  if (dobProvided) withheld.add('Full date of birth (stored as age)');

  return { record: out, withheld: [...withheld].sort() };
}

module.exports = { renderProfile, toStorableRecord, scrub, ageFromDob, MISSING, REDACTED };

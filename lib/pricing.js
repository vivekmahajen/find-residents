...
const PLANS = [
  { id: 'free', name: 'Free', monthly: 0, credits: 100, oneTime: true },
...
// Stripe billing parameters for a plan: amount in cents + recurring interval.
// Annual = 12 months at the annual discount, billed once a year.
function planBilling(plan, annual) {
  if (!plan || plan.monthly == null || plan.monthly === 0) return null;
  if (annual) {
    const yearly = Math.round(plan.monthly * 12 * (1 - ANNUAL_DISCOUNT_PCT / 100));
    return { unitAmount: yearly * 100, interval: 'year', display: yearly };
  }
  return { unitAmount: plan.monthly * 100, interval: 'month', display: plan.monthly };
}
...
// Stripe-subscribed accounts are refilled by the invoice.paid webhook instead.
// Returns true if the account changed.
function refreshCycle(acct) {
  if (acct.stripeSubscriptionId) return false; // Stripe drives renewals
...
// Refill plan credits to the plan's allotment (used on Stripe invoice.paid).
function refillPlan(acct) {
  const p = planById(acct.planId);
  acct.planCredits = (p && p.credits) || 0;
  acct.cycleStart = Date.now();
  acct.pastDue = false;
  return acct;
}
...
// Free plans (and past-due paid plans) must have the credits; current paid
// plans can always proceed via metered overage.
function restricted(acct) {
  return acct.planId === 'free' || !!acct.pastDue;
}

function canAfford(acct, key) {
  const a = ACTIONS[key];
  if (!a) return false;
  if (restricted(acct)) return acct.planCredits + acct.topupCredits >= a.inPlan;
  return true;
}

// Charge for a completed action. Deduct in-plan credits (plan first, then
// top-up); if depleted on a current paid plan, meter the overage as $ owed.
function charge(acct, key) {
  ...
  if (restricted(acct)) return { ok: false, reason: 'out_of_credits' };
...
function accountView(acct) {
  ...
  return {
    ...
    creditsUsedThisCycle: Math.max(0, (p.credits || 0) - acct.planCredits),

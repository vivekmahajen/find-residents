...
async function fetchRowsFrom(source, { q, limit } = {}) {
  if (source.resourceId) {
    ...
    const r = await fetch(url, { headers: { ...FETCH_HEADERS, Accept: 'application/json' } });
    if (!r.ok) throw new Error(`CDSS CKAN ${r.status}`);
    const j = await r.json();
    return (j && j.result && j.result.records) || [];
  }
  if (source.url) {
    const r = await fetch(source.url, { headers: { ...FETCH_HEADERS, Accept: 'text/csv, application/json' } });

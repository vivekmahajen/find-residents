...
async function init() {
  ...
  await p.query(`CREATE TABLE IF NOT EXISTS records (
    id text PRIMARY KEY,
    agency_id text,
    kind text NOT NULL,
    data jsonb NOT NULL,
  ...
  await p.query('CREATE INDEX IF NOT EXISTS records_agency_kind_idx ON records (agency_id, kind)');
  await p.query('CREATE INDEX IF NOT EXISTS records_kind_idx ON records (kind)');
  initDone = true;
}
...
function rowToRecord(r) {
  if (!r) return null;
  return { id: r.id, agencyId: r.agency_id, kind: r.kind, data: r.data, createdAt: Number(r.created_at), updatedAt: Number(r.updated_at) };
}
async function createRecord({ agencyId, kind, data }) {
  const id = crypto.randomUUID();
  const now = Date.now();
  await getPool().query(
    'INSERT INTO records (id, agency_id, kind, data, created_at, updated_at) VALUES ($1,$2,$3,$4,$5,$6)',
    [id, agencyId || null, kind, JSON.stringify(data || {}), now, now]
  );
  return { id, agencyId, kind, data: data || {}, createdAt: now, updatedAt: now };
}
async function listRecords(agencyId, kind) {
  const { rows } = await getPool().query(
    'SELECT * FROM records WHERE agency_id = $1 AND kind = $2 ORDER BY created_at DESC',
    [agencyId, kind]
  );
  return rows.map(rowToRecord);
}
async function listAllRecords(kind) {
  const { rows } = await getPool().query('SELECT * FROM records WHERE kind = $1', [kind]);
  return rows.map(rowToRecord);
}
async function getRecord(id) {
  const { rows } = await getPool().query('SELECT * FROM records WHERE id = $1', [id]);
...
async function updateRecord(id, patch) {
  ...
  const now = Date.now();
  await getPool().query('UPDATE records SET data = $1, updated_at = $2 WHERE id = $3', [JSON.stringify(data), now, id]);
  return { ...cur, data, updatedAt: now };
}
async function deleteRecord(id) {
  await getPool().query('DELETE FROM records WHERE id = $1', [id]);
  return true;
}
